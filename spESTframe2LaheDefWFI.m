...
#
#%%%%%%%%%%%%%%%%%%%%
# --------Dsplacements and  forces numbers at the end and beginning of element.  ----- -----
slmnr=0;
for i=1:NEARV
 for j=1:12
   slmnr=slmnr+1;
...
disp('  ')
disp(' sumX=p3*h08+F1-F2+X(85)+X(88)+X(90)')
sumX=p3*h08+F1-F2+X(85)+X(88)+X(90)
disp('  ')
disp(' Sum of the moments acting  about point 1: ')
disp(' sumM1=-p3*h08*h08/2-q3*(l1+1.0)*((l1+1.0)/2-1.0)-F1*0.6*h... ')
disp('       -F3*(l1+0.4*l2)+F2*0.5*h+X(87)+X(92)-l1*X(89)-(l1+l2)*X(91) ')
sumM1=-p3*h08*h08/2-q3*(l1+1.0)*((l1+1.0)/2-1.0)-F1*h1-F3*(l1+0.4*l2)+...
F2*0.5*h+X(87)+X(92)-l1*X(89)-(l1+l2)*X(91)
disp('  ')
disp('  Sum of the moments acting  about point 5:  ')
disp(' sumM7=-p3*h08*h08/2+q3*(l1+1.0)*((l1+1.0)/2+l2)-F1*0.6*h... ')
disp('       +F3*(0.6*l2)+F2*0.5*h+X(87)+X(92)+l1*X(89)+(l1+l2)*X(86) ')
sumM7=-p3*h08*h08/2+q3*(l1+1.0)*((l1+1.0)/2+l2)-F1*0.6*h+F3*(0.6*l2)+F2*0.5*h+X(87)+X(92)+l1*X(89)+(l1+l2)*X(86)
disp('  ')
...
#         axis([-d Lpikk+d  -Lpikk-0.2*d d], "ij")
   ...
   jaotT=2;
   #
   title('spESTframe2DefWFI',"fontsize",12)
   ax=gca();
...
#
#print('-deps','spESTframe2DefWFI.eps');
#print('-dpng','spESTframe2DefWFI.png');
print('-dfig','-landscape','-mono','-solid','spESTframe2DefWFI.fig');
refresh
#
#%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure(2)
...
#
#print('-deps','spESTframe2DefWFI.eps');
#print('-dpng','spESTspESTframe2DefWFI.png');
print('-dfig','-landscape','spESTframe2DefWFI_sparse_matrix.fig');
%%print('-dfig','-landscape','-mono','-solid','spESTframe2DefWFI_sparse_matrix.fig');
refresh
#
# octave
disp('-----------------------------------')
