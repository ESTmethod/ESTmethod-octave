...
#                                        N, Q, M hinges at beginning of the element.
	EIp EAp GAp   2     1    0 0 0    0 0 0;    % element 1
	EIr EAr GAr   3     2    0 0 0    0 0 0;    % element 2	
	EIr EAr GAr   4     2    0 0 0    0 0 0;    % element 3
	EIp EAp GAp   5     4    0 0 1    0 0 0;    % element 4
...
p3=p3
disp('  ')
disp(' sumZ=q3*(l1+1.0)+F3+X(86)+X(89)+X(91) ')
sumZ=q3*(l1+1.0)+F3+X(86)+X(89)+X(91)
disp('  ')
disp(' sumX=p3*h+F1-F2+X(85)+X(88)+X(90)')
sumX=p3*h+F1-F2+X(85)+X(88)+X(90)
disp('  ')
disp(' The sum of the moments acting  about point 1: ')
disp(' sumM1=-p3*h*h/2-q3*(l1+1.0)*((l1+1.0)/2-1.0)-F1*0.5*h... ')
disp('       -F3*(l1+0.4*l2)+F2*0.6*h+X(87)+X(92)-l1*X(89)-(l1+l2)*X(91) ')
sumM1=-p3*h*h/2-q3*(l1+1.0)*((l1+1.0)/2-1.0)-F1*h1-F3*(l1+0.4*l2)+...
F2*0.6*h+X(87)+X(92)-l1*X(89)-(l1+l2)*X(91)
disp('  ')
disp('  The sum of the moments acting  about point 5:  ')
disp(' sumM7=-p3*h*h/2+q3*(l1+1.0)*((l1+1.0)/2+l2)-F1*0.5*h... ')
disp('       +F3*(0.6*l2)+F2*0.6*h+X(87)+X(92)+l1*X(89)+(l1+l2)*X(86) ')
sumM7=-p3*h*h/2+q3*(l1+1.0)*((l1+1.0)/2+l2)-F1*0.5*h+F3*(0.6*l2)+F2*0.6*h+X(87)+X(92)+l1*X(89)+(l1+l2)*X(86)
disp('  ')
...
#         axis([-d Lpikk+d  -Lpikk-0.2*d d], "ij")
   ...
   jaotT=2;
   #
   title('spESTframe4DefWFI',"fontsize",12)
   ax=gca();
...
#
#print('-deps','spESTframe4DefWFI.eps');
#print('-dpng','spESTframe4DefWFI.png');
print('-dfig','-landscape','-mono','-solid','spESTframe4DefWFI.fig');
refresh
#
#%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure(2)
...
#
#print('-deps','spESTframe4DefWFI.eps');
#print('-dpng','spESTspESTframe4DefWFI.png');
print('-dfig','-landscape','spESTframe4DefWFI_sparse_matrix.fig');
%%print('-dfig','-landscape','-mono','-solid','spESTframe4DefWFI_sparse_matrix.fig');
refresh
#
# octave
disp('-----------------------------------')
