...
esFjoud(1,1:3,3)=[0.0 0.0 L3];##sinA5=(0.2*h)/L5;
...
sumM1=-p3*h*h/2-q3*(l1+1.0)*((l1+1.0)/2-1.0)-F1*0.5*h-F3*(l1+0.4*l2)+...
...
sumM7=-p3*h*h/2+q3*(l1+1.0)*((l1+1.0)/2+l2)-F1*0.5*h+F3*(0.6*l2)+F2*0.5*h+X(87)+X(90)+l1*X(89)+(l1+l2)*X(86)
disp('  ')
...
#         axis([-d Lpikk+d  -Lpikk-0.2*d d], "ij")
   ...
   jaotT=2;
   #
   title('spESTframe5DefWFI',"fontsize",12)
   ax=gca();
...
#
#print('-deps','spESTframe5DefWFI.eps');
#print('-dpng','spESTframe5DefWFI.png');
print('-dfig','-landscape','-mono','-solid','spESTframe5DefWFI.fig');
refresh
#
#%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure(2)
...
refresh
#
#print('-deps','spESTframe5DefWFI_h6remaatriks.eps');
#print('-dpng','spESTframe5DefWFI_h6remaatriks.png');
print('-dfig','-landscape','spESTframe5DefWFI_sparse_matrix.fig');
%%print('-dfig','-landscape','-mono','-solid','spESTframe5DefWFI_sparse_matrix.fig');
refresh
#
# octave
disp('-----------------------------------')
