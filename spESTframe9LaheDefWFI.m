...
##disp('  ')
#disp('---  ')
##disp(' http://digi.lib.ttu.ee/opik_eme/slaidid/raamEST1en.pdf ')
...
#GAp=0.4*4.6*10^6;
GAr=0.4*EAr;
disp('--------- Scaling multiplier for the displacements----------')
#baasi0=EIp/4.0  # scaling multiplier for the displacements
#%baasi0=1.0;
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
disp(' The displacements and the forces calculated on the parts of the element ''Nmitmeks''.  ')
Nmitmeks=4
...
L1=h;
##L2=l1;
L2=1.0; # length of the element
L3=l1; # length of the element
L4=h;
L5=l2; # length of the element
L6=sqrt(1.0^2+h^2); # length of the element
L7=1.0; # length of the element
#sqrt(h^2+l4^2);
...
#disp('    qz     qx     qA      qL ')
# The uniformly distributed load in local coordinate z and x direction kN/m^2
LoadsqONelement=4;
esQkoormu=zeros(LoadsqONelement,4,ElementideArv);
esQkoormus(1,1:4,1)=[q1 0.0 0.0 L1];
esQkoormus(1,1:4,2)=[qz2 0.0 0.0 L2];
esQkoormus(1,1:4,3)=[qz3 0.0 0.0 L3];
...
#esQkoormus
#
# The point load in local coordinate z and x direction  kN
...
esFjoud(1,1:3,5)=[Fz5 0.0 aF5];
esFjoud(1,1:3,6)=[Fz6 Fx6 aF6];
...
#%%%%%
# The support shift - tSiire#
# The support shift multiplied by scaling multiplier
tSiire = zeros(3,1,SolmedeArv);
#tSiire(1,1,1)= 0.0
#tSiire(2,1,1)= 0.01*baasi0
##%%%%
case{2}
disp(' The load variant 2 ')
#
disp(' Initial data  ')
l=8.0
h=4.0
...
krdn=[#	 x                z
	...
	l1                 -h;   % node 4
        l1                0.0;   % node 5
        l1+l2              -h;   % node 6
        l1+l2+1.0         0.0;   % node 7
        l1+l2+1.0         -h];   % node 8
#==========
 # disp(' Nr      toel  u  w  fi  N   Q   M ')
#==========
#  The restrtictions on the support displacements.
...
#
disp('====================================================')
disp('  The displacements and the forces numbers (daf''s) ')
disp('  at the end and at the beginning of elements.  ')
disp('----------------------------------------------------')
selemjln;
The_displacements_and_the_forces_numbers=selemjln
#
...
disp(' Nr.  At the end:  u, w, fi, N, Q, M.  At the beginning: u, w, fi, N, Q, M.      EI         EA          GA    End Beginning  M    M')
disp('----------------------------------------------------------------------------------------------------------------------------------------')
#
for i=1:NEARV
...
%disp('--------------------------------')
disp('==================================================')
disp(' The restrtictions on the support displacements. ')
disp(' Nr    Node Nr   u  w  fi;  hold on-1, open-0')
disp('--------------------------------------------------')
#disp([tsolm])
for i=1:NTSARV
...
disp('  ')
disp('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ')
disp(' The element displacements and forces  determinated by the transfer matrix ')
disp('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ')
load_variant
disp('  ')
...
#-------------The element displacements and forces----------
   #
   VardaNr=i;
   disp(sprintf('%13s %2i %15s %5.3f %2s', 'The displacements and forces of element nr',VardaNr,'  length of element ',Li,'m,  '))
...
disp('  ')
disp(' The_support_reactions=[X(85) X(86) X(87) X(88) X(89) X(90) X(91) X(92)] ')
The_support_reactions=[X(85) X(86) X(87) X(88) X(89) X(90) X(91) X(92)]
disp('The_lengths=[l1 l2 h1  h] ')
The_lengths=[l1 l2 h1  h]
## q2=p12; 61+24=85  -- 92
disp(' q3=p1 or q3=p2  ')
q3=q3
p3=p3
disp('  ')
disp(' sumZ=q3*(l1+1.0)+F3+X(86)+X(88)+X(91) ')
sumZ=q3*(l1+1.0)+F3+X(86)+X(88)+X(91)
disp('  ')
disp(' sumX=p3*h+F1-F2+X(85)+X(87)+X(90)')
sumX=p3*h+F1-F2+X(85)+X(87)+X(90)
disp('  ')
disp(' The sum of the moments acting  about point 1: ')
disp(' sumM1=-p3*h*h/2-q3*(l1+1.0)*((l1+1.0)/2-1.0)-F1*0.6*h... ')
disp('       -F3*(l1+0.4*l2)+F2*0.6*h+X(89)+X(92)-l1*X(88)-(l1+l2+1.0)*X(91) ')
sumM1=-p3*h*h/2-q3*(l1+1.0)*((l1+1.0)/2-1.0)-F1*0.6*h-F3*(l1+0.4*l2)+...
F2*0.6*h+X(89)+X(92)-l1*X(88)-(l1+l2+1.0)*X(91)
disp('  ')
disp('  The sum of the moments acting  about point 5:  ')
disp(' sumM7=-p3*h*h/2+q3*(l1+1.0)*((l1+1.0)/2+l2+1.0)-F1*0.6*h... ')
disp('       +F3*(0.6*l2+1.0)+F2*0.6*h+X(89)+X(92)+(l2+1.0)*X(88)+(l1+l2+1.0)*X(86) ')
sumM7=-p3*h*h/2+q3*(l1+1.0)*((l1+1.0)/2+l2+1.0)-F1*0.6*h+F3*(0.6*l2+1.0)+F2*0.6*h+X(89)+X(92)+(l2+1.0)*X(88)+(l1+l2+1.0)*X(86)
disp('  ')
disp(' The calculations verified the static equilibrium of the frame   ')
disp('  ')
#%%%%%%%%%%%%
figure(1)
   ax = gca ();
...
#         axis([-d Lpikk+d  -Lpikk-0.2*d d], "ij")
   ...
   jaotT=2;
   #
   title('spESTframe9DefWFI',"fontsize",12)
   ax=gca();
...
sammuga=0.5;
#sammuga=0.8;
text(-1.0, yvalg-1.5*sammuga, 'The displacements and the forces numbers (daf''s) ')
...
text(-1.0, yvalg, 'The support reactions: 85 86 87 88 89 90 91 92 ')
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
xvalg=7.0;
#xvalg=10.0;
#yvalg=-15.5;
...
#
#print('-deps','spESTframe9DefWFI.eps');
#print('-dpng','spESTframe9DefWFI.png');
print('-dfig','-landscape','-mono','-solid','spESTframe9DefWFI.fig');
refresh
#
#%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure(2)
...
refresh
#
#print('-deps','spESTframe9DefWFI_h6remaatriks.eps');
#print('-dpng','spESTframe9DefWFI_h6remaatriks.png');
print('-dfig','-landscape','spESTframe9DefWFI_sparse_matrix.fig');
%%print('-dfig','-landscape','-mono','-solid','spESTframe9DefWFI_sparse_matrix.fig');
refresh
#
# octave
disp('-----------------------------------')
